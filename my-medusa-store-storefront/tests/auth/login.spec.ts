...
test.describe('Login', () => {
  ...
  test.describe('Happy path', () => {
    ...
    test('should login with valid credentials @auth @smoke', async ({ loginPage }) => {
    ...
    test('should persist session after page refresh @auth', async ({ loginPage }) => {
      await loginPage.login(validUser.email, validUser.password);
      await loginPage.page.reload();
      await expect(loginPage.userMenu).toBeVisible();
    });

    test('should redirect to home after logout @auth', async ({ loginPage }) => {
      await loginPage.login(validUser.email, validUser.password);
  ...
  test.describe('Validations', () => {
    ...
    test('should show error with wrong password @auth', async ({ loginPage }) => {
      await loginPage.login(invalidUser.email, invalidUser.password);
      await expect(loginPage.errorMessage).toBeVisible();
    });

    test('should show error with unregistered email @auth', async ({ loginPage }) => {
