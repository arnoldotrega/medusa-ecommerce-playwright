...
test.describe('Auth API @api', () => {
  ...
  test('POST /store/auth with valid credentials returns 200 @api', async ({ request }) => {
  ...
  test('POST /store/auth with wrong password returns 401 @api', async ({ request }) => {
    const response = await request.post(API_URL + '/store/auth', {
  ...
  test('POST /store/auth with missing fields returns 400 @api', async ({ request }) => {
    const response = await request.post(API_URL + '/store/auth', {
      data: {}
    });
