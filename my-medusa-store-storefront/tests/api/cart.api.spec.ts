...
test.describe('Cart API @api', () => {
  ...
  test('POST /store/carts/:id/line-items adds item to cart @api', async ({ request }) => {
  ...
  test('POST /store/carts/:id/line-items with invalid variant returns 400 @api', async ({ request }) => {
    const cartResponse = await request.post(API_URL + '/store/carts');
    const cartData = await cartResponse.json();
    const cartId = cartData.cart.id;
