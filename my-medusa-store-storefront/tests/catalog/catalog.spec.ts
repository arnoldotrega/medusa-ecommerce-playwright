...
test.describe('Product Catalog', () => {
  test.describe('Happy path', () => {
    test('should display products on store page @catalog @smoke', async ({
      productListPage,
    }) => {
    ...
    test('should find product by valid search term @catalog', async ({
      homePage,
      productListPage,
    }) => {
    ...
    test('should filter products by category @catalog', async ({
      productListPage,
    }) => {
      await productListPage.navigate('/store')
    ...
    test('should sort products by price low to high @catalog', async ({
      productListPage,
    }) => {
      await productListPage.navigate('/store')
    ...
    test('should open product detail page on click @catalog', async ({
      productListPage,
      productDetailPage,
    }) => {
      await productListPage.navigate('/store')
  ...
  test.describe('Validations', () => {
    test('should show no results for unknown search term @catalog', async ({
      homePage,
      productListPage,
    }) => {
      await homePage.navigate('/')
    ...
    test('should handle XSS in search without crashing @catalog', async ({
      homePage,
    }) => {
      await homePage.navigate('/')
    ...
    test('should show 404 for non-existent product URL @catalog', async ({
      productDetailPage,
    }) => {
