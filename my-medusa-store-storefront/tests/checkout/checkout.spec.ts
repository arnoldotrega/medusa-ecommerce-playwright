...
import { validAddress, validCard, invalidCard, invalidCoupon } from '../utils/data/checkout';
...
test.describe('Checkout', () => {
  ...
  test.describe('Happy path', () => {
    ...
    test('should fill shipping address successfully @checkout', async ({ cartPage, checkoutPage }) => {
    ...
    test('should complete full guest checkout @checkout @smoke', async ({ cartPage, checkoutPage }) => {
      await cartPage.navigate('/cart');
      await cartPage.proceedToCheckout();
      await checkoutPage.fillShippingAddress(validAddress);
  ...
  test.describe('Validations', () => {
    ...
    test('should show errors on empty shipping form @checkout', async ({ cartPage, checkoutPage }) => {
      await cartPage.navigate('/cart');
      await cartPage.proceedToCheckout();
      await checkoutPage.placeOrder();
    ...
    test('should reject expired credit card @checkout', async ({ cartPage, checkoutPage }) => {
      await cartPage.navigate('/cart');
      await cartPage.proceedToCheckout();
      await checkoutPage.fillShippingAddress(validAddress);
    ...
    test('should show error for invalid coupon @checkout', async ({ cartPage, checkoutPage }) => {
      await cartPage.navigate('/cart');
      await cartPage.proceedToCheckout();
    ...
    test('should not allow checkout with empty cart @checkout', async ({ cartPage }) => {
      await cartPage.navigate('/cart');
