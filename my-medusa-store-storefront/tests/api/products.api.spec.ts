...
test.describe('Products API @api', () => {
  ...
  test('GET /store/products returns array of products @api', async ({
    request,
  }) => {
    const response = await request.get(API_URL + '/store/products')
  ...
  test('GET /store/products/:id with valid id returns 200 @api', async ({
    request,
  }) => {
  ...
  test('GET /store/products/:id with fake id returns 404 @api', async ({
    request,
  }) => {
